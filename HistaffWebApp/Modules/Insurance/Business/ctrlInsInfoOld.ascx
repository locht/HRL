...
<tlk:RadScriptBlock ID="scriptBlock" runat="server">
    <script type="text/javascript">
        ...
        function GridCreated(sender, eventArgs) {
            registerOnfocusOut('ctl00_MainContent_ctrlInsInfoOld_RadSplitter1');
        }
        ...
        function OpenInsertWindow() {
            window.open('/Default.aspx?mid=Insurance&fid=ctrlInsInfoOldNewEdit&group=Business&FormType=0', "_self"); /*
        ...
        function OpenEditWindow() {
            ...
            if (id > 0) {
                window.open('/Default.aspx?mid=Insurance&fid=ctrlInsInfoOldNewEdit&group=Business&VIEW=TRUE&FormType=0&ID=' + id + '', "_self"); /*
                oWindow.setSize(800, 480);
                oWindow.center(); */
            }
        }
        ...
        function OnClientButtonClicking(sender, args) {
            if (args.get_item().get_commandName() == 'CREATE') {
                ...
                var rows = $find('<%= rgInsInfoOld.ClientID %>').get_masterTableView().get_dataItems().length;
                if (rows == 0) {
