<%@ Control Language="vb" AutoEventWireup="false" CodeBehind="ctrlRC_DBStatisticGender.ascx.vb"
    Inherits="Recruitment.ctrlRC_DBStatisticGender" %>
...
<div class="chart-top" Style="margin-left: 10px">
    <%# Translate("Năm: ")%> <%=_year %>
    <tlk:RadComboBox ID="cboType" SkinID="dDropdownList" runat="server" AutoPostBack="true" width = "80px" OnClientSelectedIndexChanged="OnClientClicked" Visible="false">
    </tlk:RadComboBox>
...
<div class="box-headchart">
    <span class="headchart"><%# Translate("Số lượng ứng viên theo giới tính")%></span>
</div>
    ...
    function MaxClick(sender, e) {
        ...
        switch (btnText) {
            case "Maximize":
                ...
                oWnd.setUrl('Dialog.aspx?mid=Recruitment&fid=ctrlRC_DBStatisticGender&group=Dashboard&width=' + (winW) + '&height=' + (winH - 80) + "&noscroll=1&reload=1&resize=" + 1);
                break;
            case "Restore":
                oWnd.restore();
