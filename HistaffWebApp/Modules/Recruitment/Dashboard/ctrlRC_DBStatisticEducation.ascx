<%@ Control Language="vb" AutoEventWireup="false" CodeBehind="ctrlRC_DBStatisticEducation.ascx.vb"
    Inherits="Recruitment.ctrlRC_DBStatisticEducation" %>
...
<div class="chart-top" Style="margin-left: 10px">
    <%# Translate("Năm: ")%>
    <%=_year %>
    <tlk:RadComboBox ID="cboType" runat="server" AutoPostBack="true" Width="80px" OnClientSelectedIndexChanged="OnClientClicked"
        Visible="false">
    </tlk:RadComboBox>
...
<div class="box-headchart">
    <span class="headchart">
        <%# Translate("Số lượng ứng viên theo trình độ học vấn")%></span>
</div>
    ...
    function MaxClick(sender, e) {
        ...
        switch (btnText) {
            case "Maximize":
                ...
                oWnd.setUrl('Dialog.aspx?mid=Recruitment&fid=ctrlRC_DBStatisticEducation&group=Dashboard&width=' + (winW) + '&height=' + (winH - 80) + "&noscroll=1&reload=1&resize=" + 1);
                break;
            case "Restore":
                oWnd.restore();
